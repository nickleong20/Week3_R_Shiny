...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      downloadButton("donwloadData", "Download Example Dataset")  # Add the Download button
    ),
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Help", 
                 ...
                 p("Common Issues:"),
                 tags$ol(
                   tags$li("Ensure you upload a file with one of the supported extensions: CSV, ASP, JDX, SPC, SPA, 0."),
                   tags$li("For intensity smoothing and background correction, input valid values within the specified ranges (1 to 7 for smoothing, 1 to 20 for background)."),
                   tags$li("If the results table is empty, the uploaded spectrum may not match any spectra in the FTIR library."),
                   tags$li("Check your internet connection and ensure all required packages are installed and loaded correctly."),
                   tags$li("Large datasets or a high number of combinations can increase processing time."),
                   tags$li("Refer to the app's documentation and error messages for additional troubleshooting."),
                   tags$li("In case of R session crashes, consider using smaller datasets or optimizing system resources.")
                 )
        )
      )
    )
...
server <- function(input, output) {
  ...
  observeEvent(input$file, {
    ...
    # Download handler to generate and download the example dataset
    output$downloadData <- downloadHandler(
      filename = function() {
        "example_dataset.csv"  # Set the filename for the downloaded dataset
      },
      content = function(file) {
        # Call the create_example_dataset function to get the data
        example_dataset <- create_example_dataset()
        # Write data to CSV file
        write.csv(example_dataset, file, row.names = FALSE)
      }
    )
